...
public class V1OrdinaryResource {
    ...
    @ApiResponses(value = { @ApiResponse(code = 200, message = "OK", response = V1OrdinaryResponse.class),
            @ApiResponse(code = 401, message = ""), @ApiResponse(code = 403, message = ""),
            @ApiResponse(code = 400, message = "Bad Request"),
            @ApiResponse(code = 500, message = "Internal Server Error") })
    public V1OrdinaryResponse requestAsAdmin(@ApiParam(required = true) @QueryParam("req") @NotNull String req)
            throws Exception {
        return request(req);
    }
    ...
    @ApiResponses(value = { @ApiResponse(code = 200, message = "OK", response = V1OrdinaryResponse.class),
            @ApiResponse(code = 400, message = "Bad Request"),
            @ApiResponse(code = 500, message = "Internal Server Error") })
    public V1OrdinaryResponse request(@ApiParam(required = true) @QueryParam("req") @NotNull String req)
            throws Exception {
